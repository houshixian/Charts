...
@interface TwoLeftLineCountView ()
{
...
@implementation TwoLeftLineCountView
+ (void)load
{
...
- (CGRect)getHintViewFrame:(CGRect)frame location:(CGPoint)location radius:(CGFloat)radius
{
    ...
    if ([_lineData.lines count] >= 4 && [_lineData.lines count] < 6) {
        int i = rand()%20;
        frame.origin.y = 70 + i ;
    }else if ([_lineData.lines count] >= 6 && [_lineData.lines count] < 8){
        int i = rand()%20;
        frame.origin.y = 20 + i ;
    }else if ([_lineData.lines count] >= 8 ){
        int i = rand()%5;
        frame.origin.y = -10 + i ;
    }else{
        frame.origin.y = location.y - 18;
...
{
    if (hintView && line) {
        CGRect frame = hintView.frame;      //9.23 显示位置
...
- (void)adjustHintViewPosition:(ISSChartHintView*)hintView radius:(CGFloat)radius
{
    ...
    if (CGRectGetMaxX(hintView.frame) > CGRectGetWidth(self.bounds)) {
        lineHintView.backgroundImageView.transform = CGAffineTransformScale(lineHintView.backgroundImageView.transform, -1, 1);
...
{
    ISSChartLine *line = [self findSelectedLineWithLocation:location];      //9.23 线的显示图
    if (line) {
        ...
        if (NSNotFound != index) {
            ...
            if(NSNotFound == _previousIndex||_previousIndex != index) {
                if (_didSelectedLines) {
                    ISSChartAxisItem *axisItem = [_lineData xAxisItemAtIndex:index];
                    hintView = _didSelectedLines(self, self.lineData.lines, index, axisItem);
                    if (hintView) {
...
- (void)handleSingleTap:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    if (line) {
        ...
        if (NSNotFound != index) {
            ...
            if (_didSelectedLines) {
                ISSChartAxisItem *axisItem = [_lineData xAxisItemAtIndex:index];
                _didSelectedLines(self, self.lineData.lines, index, axisItem);
            }
        }
    }
}
