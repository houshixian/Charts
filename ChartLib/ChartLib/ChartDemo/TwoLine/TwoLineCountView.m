...
@end
@implementation TwoLineCountView
...
{
    _histogramLineData = histogramLineData;
    _chartData = histogramLineData;
...
- (BOOL)isPointOnBarWithX:(ISSChartHistogramBar*)bar location:(CGPoint)location
{
    ...
    rect.size.height = (rect.size.height == 0)?1:rect.size.height;
...
- (void)adjustHintViewPosition:(ISSChartHintView*)hintView offset:(CGSize)offset
{
    ...
    histogramLineHintView.backgroundImageView.transform = CGAffineTransformIdentity;
    if (CGRectGetMaxX(hintView.frame) >= CGRectGetWidth(self.bounds)) {
        histogramLineHintView.backgroundImageView.transform = CGAffineTransformScale(histogramLineHintView.backgroundImageView.transform, -1, 1);
...
- (void)handleSingleTap:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    if (bar) {
        if (_didSelectedBarBlock) {
            ...
            if (bar) {
                NSInteger indexOfValueOnLine = bar.groupIndex;
                _didSelectedBarBlock(self, bar, self.histogramLineData.lines, indexOfValueOnLine, [_histogramLineData getXAxisItemWithBar:bar]);
            }
        }
    }
}
...
{
    ITTDINFO(@"panGestureRecognizer.state %ld", (long)gestureRecognizer.state);
