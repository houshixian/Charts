...
//    [self drawPatternInRect:bar.barProperty.frame context:context bar:bar];
    ...
    frame.size.height = (frame.size.height < 1 &&frame.size.height >=0)?1:frame.size.height;
    ...
    if (2 == gradientColorCount) {
        ...
        CGPoint endPoint = CGPointMake(CGRectGetMaxX(frame), CGRectGetMinY(frame));
		...
		CGContextAddPath(context, borderPath);
        CGContextClip(context);
        CGPoint startPoint = CGPointMake(frame.origin.x, frame.origin.y);   //7.28
        CGPoint endPoint = CGPointMake(frame.size.width+frame.origin.x, frame.origin.y);
