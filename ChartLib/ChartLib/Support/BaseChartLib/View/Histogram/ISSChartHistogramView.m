...
- (void)differentGroupSizeDataUpdateAnimation:(NSTimer*)timer
{
    ...
    if (final) {
        ...
        _histogramContentView = newHistogramContentView;
...
- (BOOL)isPointOnBarWithX:(ISSChartHistogramBar*)bar location:(CGPoint)location
{
    BOOL intersects = FALSE;
    CGRect rect = bar.barProperty.frame;
    rect.size.height = (rect.size.height == 0)?self.bounds.size.height:rect.size.height;
...
- (void)adjustHintViewPosition:(ISSChartHintView*)hintView offset:(CGSize)offset
{
	...
	histogramHintView.backgroundImageView.transform = CGAffineTransformIdentity;
    if (CGRectGetMaxX(hintView.frame) >= CGRectGetWidth(self.bounds)) {
        histogramHintView.backgroundImageView.transform = CGAffineTransformScale(histogramHintView.backgroundImageView.transform, -1, 1);
...
- (void)filterVisiableAreaByValue:(CGFloat)minValue maxValue:(CGFloat)maxValue
{
    ...
    _minimumValueFilterView = [self getFilterView:miniFileterImage marginY:CGRectGetHeight(self.bounds) - height top:FALSE];
    height = _histogramData.coordinateSystem.topMargin;
    _maxmumValueFilterView = [self getFilterView:nil marginY:height - CGRectGetHeight(self.bounds) top:TRUE];
...
- (void)handleSingleTap:(UITapGestureRecognizer *)gestureRecognizer
{
    ...
    ISSChartHistogramBar *bar = [self findBarIndexWithLocatin:location];
    ISSChartAxisItem *xItem =  _histogramData.coordinateSystem.xAxis.axisItems[bar.groupIndex];
    NSString *name = xItem.name;
    
    
    if (bar && [_delegate respondsToSelector:@selector(tapItem:)]) {
//		if (_didSelectedBarBlock) {
//			_didSelectedBarBlock(self, bar, [_histogramData getXAxisItemWithBar:bar]);
//		}
        [_delegate tapItem:@{@"name":name}];
    }
}
